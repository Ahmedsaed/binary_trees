 ...
 * @i: node index
 * @nodes_len: number of nodes
 *
 *  Return: 1 if tree is complete, 0 otherwise
 */
int tree_is_complete(const binary_tree_t *tree, int i, int nodes_len)
{
	if (tree == NULL)
		return (1);
	...
	return (tree_is_complete(tree->left, (2 * i) + 1, nodes_len) &&
		tree_is_complete(tree->right, (2 * i) + 2, nodes_len));
}
...
int binary_tree_is_complete(const binary_tree_t *tree)
{
	...
	nodes_len = binary_tree_size(tree);

	return (tree_is_complete(tree, 0, nodes_len));
}
